...
class ADM_ShopUIBarterItemIcon : ScriptedWidgetComponent
{
	...
	void OnHoverDetected()
	{
		...
		if (m_PaymentMethod) {
			int quantity = ADM_ShopUI_old.FindShopUIItem(m_wRoot).GetQuantity();
...
class ADM_ShopUI_old: ChimeraMenuBase
{
	...
	void SelectCategory(int index = 0)
	{
		...
		array<ref ADM_ShopCategory> categories = m_Shop.GetCategories();
		array<ref ADM_ShopMerchandise> items = m_Shop.GetMerchandiseBuy();
	...
	void ProcessSearch(SCR_EditBoxSearchComponent searchbox, string search)
	{
		...
		foreach (ADM_ShopMerchandise merch : m_Shop.GetMerchandiseBuy())
		{
